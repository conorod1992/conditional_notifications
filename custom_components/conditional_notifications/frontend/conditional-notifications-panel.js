...
class ConditionalNotificationsPanel extends HTMLElement {
  ...
  async action(id, action) {
    ...
    try {
      ...
      this.showToast(action === "test" ? "Test notification sent" : `${action[0].toUpperCase()}${action.slice(1)} complete`);
  ...
  preview(d) {
    ...
    const conditions = d.conditions?.length ? `${d.conditions.length} condition${d.conditions.length===1?"":"s"} must pass` : "No extra conditions";
  ...
  async save() {
    ...
    try {
      if (this.editor.id) await this.hass.callWS({type:`${WS}/update`,notification_id:this.editor.id,changes:d});
      else await this.hass.callWS({type:`${WS}/create`,definition:d});
      this.dirty = false; this.closeEditor(true); this.showToast(this.editor?.id ? "Changes saved" : "Notification created"); await this.refresh();
    } catch (error) { this.showToast(error.message || String(error)); }
  }
  ...
  renderCard(record) {
    ...
    return `<article class="card" tabindex="0" data-open="${record.id}"><div class="card-top"><div class="status-icon ${record.status}">●</div><div class="grow"><h3>${esc(record.name)}</h3><p>${esc(trigger)}</p></div><span class="badge">${esc(record.status)}</span><button class="icon menu" data-menu="${record.id}" aria-label="Actions">•••</button></div><div class="meta"><span>🔔 ${record.notification_count} sent${record.remaining_notifications!==null?` · ${record.remaining_notifications} remaining`:""}</span><span>◷ ${esc(ready)}</span>${record.definition.expires_at?`<span>⌛ ${fmt(record.definition.expires_at)}</span>`:""}</div><div class="quick"><button data-action="${record.paused?"resume":"pause"}" data-id="${record.id}">${record.paused?"Resume":"Pause"}</button><button data-action="test" data-id="${record.id}">Test</button><button data-edit="${record.id}">Edit</button><button class="danger" data-action="delete" data-id="${record.id}">Delete</button></div></article>`;
  }
  filtered() {
